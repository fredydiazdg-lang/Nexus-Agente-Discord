...
# Configuración ultra compatible sin bloqueos de formato
YTDL_OPTIONS = {
...
def extraer_audio(query):
    """Extrae la información de la canción mediante clientes mweb/android de YouTube."""
...
def reproducir_siguiente(vc, guild_id, bot):
    """Gestor de cola y reproducción continua."""
    ...
    try:
        ...
        if guild_id in active_message and active_message[guild_id]:
            ...
            embed.add_field(name="Volumen:", value=f"`{int(volumes.get(guild_id, 1.0) * 100)}%`", inline=True)
...
class Musica(commands.Cog):
    ...
    async def play(self, interaction: discord.Interaction, busqueda: str):
        ...
        try:
            data = await asyncio.to_thread(extraer_audio, busqueda_limpia)
